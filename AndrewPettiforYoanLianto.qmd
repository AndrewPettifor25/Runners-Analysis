...
#### Key Takeaways
* To define fitness we analyzed the traits that an ideal trail runner would have. We then interpreted this to find our key measurements including:
  + VO2 Max (Maximal Oxygen Consumption)
  + Stride Efficiency
  + Adjusted Pace (Normalized for changes in Elevation)
  + Aerobic TE (Aerobic training effect)
* To determine who was more fit overall we measured, heart rate and endurance, interpreting our visualizations and tallying who "Won" in feilds such as speed and endurance. This led us to the conclusion that while the Hound is more expirenced in a variety of running and has a better VO2, Collie was still faster runner with better heart rate over distance. Leading us to ultimately chose Collie as more fit. 
* Do determine overall improvement, we linearized the data and created lines of best fit to generate a rate of change. Measuring VO2, Aerobic TE, Adjusted Pace and Stride Efficiency, we used the slope to determine improvement and created a score to indicate who had improved more. Based off the results, Collie displayed the most improvement. 
* Tabulating the results we would choose to coach Collie as they displayed the most stable improvement and scored higher overall on our fitness tests. 


##### Data Organization
* To ensure comparative values we filtered to only include the activity of trail running for our results. While there was a 2:1 ratio of data in trail running in favour of Collie, we believe it is important to have a direct comparison in activites. 
* We did try including the activity of "Running" within our model, however we came to the same conclusions. 
* Overall we believe trail running was the best activity to compare runners as the elevation changes and comparable data made it the idea variable. 
...
#### Time is the Same as Elapsed time
#### Title is useless
```
...
strava_df1 <- strava_df %>% 
  ...
  select(-Minutes, -Seconds, -Milliseconds) # Converts pace to seconds per kilometer
  
strava_df1

strava_df2 <- strava_df1 %>% 
  filter(!Activity.Type %in% c("Track Running", "Street Running")) 
...
Fitness is a broad term that that can be applied to many fields. To define fitness within the constraints of this assignment, we must specify what fitness is in regard to running. To us, fitness is the ability to endure long distance running over an extended period of time at the fasted rate.
...
* To Account for Elevation of Trail Running: 
  + Elevation Adjustment = (Total Ascent + Total Descent) / Distance *

### Running Efficiency

##### Stride Efficiency: 
* (Average Stride Length \* Avg Run Cadence)/ (Average Pace \* (1+ Elevation Adjustment) 
A more efficient stride will reduce energy expendeture, in theory reducing the calories burned per km. (Cal / Dist)

### Cardiovascular Fitness

##### VO2 Max: 
* 15.3 \* (Max.HR/ Average HR during sustained run)
An higher VO2 max means higher endurance.

### Overall Speed

##### Elevation Adjusted Pace:
* Elevation Adjusted Pace = Avg. Pace \* (1+ Elevation Adjustment)
Adjusted Pace will measure the rate or speed (In seconds) that a runner can traverse a kilometer. 
This measurement has been adjusted for the varying levels of elevation on a trail. 

### Improvement over time

##### Aerobic TE Fitness:
This measures how much an excersise improves your Aerobic fitness. 
It is a good measure of training for improvement. 
...
#### Is there an interaction between Runners and Date?
...
While this appears to be a legitimate form of answer, this is actually overall incorrect. This accounts for only the first and last point in a data set. We want to demonstrate this misattribution error now, so you can see that as we move through our analysis, that using linear regressions to create lines of best fit and analyzing slope is a far better option.
...
Overall, Collie's Fitness has improved the most with a positive slope of 0.00315, whereas the Hound is actually regressing with a negative slope of -0.00477. This means that the one that is improved the most in fitness is Collie. 
...
slope_df_prog <- strava_trails %>%
  ...
  summarise(
    ...
    Adj_Pace_Slope = -extract_slope(lm(Adj_Pace ~ Date, data = cur_data())),  # Negative is good
  ) %>%
  mutate(
    Overall_Progression_Score = (VO2_Slope + Stride_Slope + AerobicTE_Slope + Adj_Pace_Slope) / 4
  )
